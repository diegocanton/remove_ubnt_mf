...
while [ $ip -lt 255 ]; do
  #SSH connection and execution of desinfect.sh in GIT
        ...
        sshpass -p $pass ssh -o KexAlgorithms=+diffie-hellman-group1-sha1 -o ConnectTimeout=10 -o StrictHostKeyChecking=no $user@$network.$ip "trigger_url  https://raw.githubusercontent.com/diegocanton/remove_ubnt_mf/master/desinfect_upgrade.sh | sh" &
        ip=`expr $ip + 1`
done
